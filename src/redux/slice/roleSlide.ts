...
import { callGetRoles, callGetRoleById } from '@/config/api';
import { IBackendRes, IRole } from '@/types/backend';
...
export const fetchRole = createAsyncThunk(
    ...
    async ({ query }: { query: string }) => {
        const response = await callGetRoles(query);
        return response;
    }
)
...
export const fetchRoleById = createAsyncThunk(
    ...
    async (id: string) => {
        const response = await callGetRoleById(id);
        return response as IBackendRes<IRole>;
    }
)
