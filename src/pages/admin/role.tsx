...
import { IBackendError, IPermission, IRole } from "@/types/backend";
import { DeleteOutlined, EditOutlined, PlusOutlined } from "@ant-design/icons";
import { ActionType, ProColumns } from '@ant-design/pro-components';
import { Button, Popconfirm, Space, Tag, message, notification } from "antd";
import { useState, useRef, useEffect } from 'react';
import dayjs from 'dayjs';
import { callDeleteRole, callGetPermissions } from "@/config/api";
...
import { groupByPermission, isSuccessResponse } from "@/config/utils";
...
const RolePage = () => {
    ...
    useEffect(() => {
        const init = async () => {
            const res = await callGetPermissions(`page=1&size=100`);
            if (res.data?.result) {
    ...
    const buildQuery = (params: any, sort: any, filter: any) => {
        ...
        let sortBy = "";
        if (sort && sort.id) {
            sortBy = sort.name === 'ascend' ? "sort=id,asc" : "sort=id,desc";
        }
    ...
    return (
        <div>
            <Access
                permission={ALL_PERMISSIONS.ROLE.GET_ALL}
            >
                <DataTable<IRole>
                    actionRef={tableRef}
